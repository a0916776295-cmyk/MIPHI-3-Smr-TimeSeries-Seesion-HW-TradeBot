...
def train_and_predict_randomforest(df: pd.DataFrame, forecast_days: int):
    """
    Random Forest модель для прогнозирования цен акций
    """
    ...
    # Обучение Random Forest модели с оптимальными параметрами
    ...
    print(f"RandomForest: RMSE={rmse:.2f}, MAPE={mape:.2f}%")
    ...
    print(f"RandomForest: Clean prediction range: {future_predictions.min():.2f} - {future_predictions.max():.2f}")
    print(f"RandomForest: Visual prediction range: {future_predictions_visual.min():.2f} - {future_predictions_visual.max():.2f}")
