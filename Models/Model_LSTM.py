...
def calculate_rsi(series, period=14):
    ...
    rs = gain / (loss + 1e-8)
    rsi = 100 - (100 / (1 + rs))
    return rsi.fillna(50).values.flatten()  # Заполняем нейтральным значением 50
...
def train_and_predict_lstm(df: pd.DataFrame, forecast_days: int):
    """
    LSTM модель для прогнозирования цен акций
    """
    ...
    features = pd.DataFrame({
        ...
        'RSI': calculate_rsi(close_series, 14),  # Индекс относительной силы
    })
    ...
    # Отдельный скейлер для цены закрытия (для обратного преобразования)
    ...
    # Построение улучшенной LSTM модели
    ...
    # Прогноз на тестовой выборке (БЕЗ шума для точных метрик)
    ...
    print(f"LSTM: Historical volatility: {historical_volatility:.4f}, Noise factor: {noise_factor}")
    ...
    print(f"LSTM: Prediction range: {future_predictions.min():.2f} - {future_predictions.max():.2f}")
