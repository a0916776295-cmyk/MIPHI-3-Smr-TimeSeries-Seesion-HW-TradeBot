...
def train_and_predict_xgboost(df: pd.DataFrame, forecast_days: int):
    """
    XGBoost модель для прогнозирования цен акций
    """
    ...
    # Обучение XGBoost модели с оптимальными параметрами
    ...
    print(f"XGBoost: Clean prediction range: {future_predictions.min():.2f} - {future_predictions.max():.2f}")
    print(f"XGBoost: Visual prediction range: {future_predictions_visual.min():.2f} - {future_predictions_visual.max():.2f}")
